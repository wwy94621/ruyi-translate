...
- 对复杂排版页面，会优先尝试使用 PDF 的 StructTree / marked content 恢复逻辑阅读顺序；若文档未提供可靠结构信息，则回退到基于坐标的双栏检测与分栏排序
- 双栏回退会先识别页面中明显的纵向 gutter，再按“全宽顶部 -> 左栏 -> 右栏 -> 全宽底部”的阅读流分段，避免左右栏内容交错
- 当前版本不包含 OCR，仅支持带文本层的 PDF
- 当前版本主要覆盖单栏与双栏文档；三栏及以上、纯图片 PDF、竖排文本、旋转书写仍不在支持范围内
